...
opt = SGD(lr=0.001, momentum=0.9, decay=0.01/epoch, nesterov=True)
model = MiniVGGNet.build(width=32, height=32, depth=3, classes=2)
model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=["accuracy"])

print("[INFO] training network...")
H = model.fit(trainX, labelsOneHot_train, validation_data=(testX, labelsOneHot_test),epochs=epoch, verbose=1, batch_size=64)
...
plt.legend()
plt.savefig('/home/a/Deep_Learning/output/minivggnet_animals3.png')